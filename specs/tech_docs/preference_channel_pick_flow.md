...
P2＋P3 全部完成后按分块方案部署生产：`package`（TAG=v20260908-1848-gdcafab0，
78MB tar）→ 8MB×10 分块 scp（单块重试×3）→ 服务器 `cat` 拼回 → sha256
双端一致 → `up`（env 检查＋docker load＋compose up＋健康等待＋冒烟全过，
无新迁移）。部署后手动重刷池快照一次（303、34s、24,693 条，时间
2026-09-08 18:56 服务器本地）；「校验名单」按钮已在线上名单页。顺手把
`XIAOJING_IMAGE_TAG` 固化进服务器 .env（此前空值，手动 `compose up` 会落
`local` 缺省 tag）。公网 https://api.jingshanai.com/healthz `{"ok":true}`。
P2 桌面端改动随桌面版发布生效，与本次后端部署解耦。
