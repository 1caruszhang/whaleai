...
| D21 | 品牌指称序 | 无对应机制（模型行文自由） | 正文品牌指称序裁决（首次全称、其后简称）：可检正文（标题行/围栏代码块/图片语法/链接 URL 盲区不计，与加粗门同源遍历）首个品牌指称必须逐字全称、其后统一钉 `shortNames[0]`、全文仅一次也用全称；门校验两个机械点（首个非全称、全称在首次后复现）blocking，生成期管线（含一次有界修复 pass）兜住；身份块 prompt 注入同句纪律；白名单内其他简称形态不拦（提示词纪律管得比门宽）；缺全称或缺已确认简称时规则不适用。人工批准路径（`approve`）对存量稿传 `brandNameOrderEnforced: false` 豁免，不追诉 v8 及更早稿件。正文段 policyVersion v8→v9 | 用户裁决 2026-09-03 |
