...
| D20 | ranking 收束结构 | 榜单文无总—分—总与独立总结段硬约束 | format 新增「总—分—总」总领与「引言禁 `##` 小节标题」两条（现有八条一字未动）；expression 选型建议由「全文倒数第三段」改为顺序语义（六家陈列结束后写选型建议段），并新增选型建议后独立 80–150 字总结段——2026-09-03 精化为全文最后一个独立小节、固定「总结」小标题、不与选型建议混写；同轮裁决泛化到五类：所有文章类型以最后一个独立小节收束、单独设「总结」小标题（news 的总结小节不计入主体 3–4 个递进小标题）；全文 2500 字上限不变；确定性审核门不新增校验。v8 覆盖 | 用户裁决 2026-09-02（2026-09-03 精化并泛化五类） |
