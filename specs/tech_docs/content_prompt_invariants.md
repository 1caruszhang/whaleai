...
3. 格式契约（确定性可校验，正文 policyVersion v3 起）：per-type H2 下限（guide/showcase 3、ranking 6、news 两类 2）、品牌名加粗全覆盖（2026-09-01 起为管线自动加粗 + 门断言；段落长度等表达层要求 2026-08-18 起不机械拦截）、品牌指称序（2026-09-03 起：首次全称、其后钉第一个已确认简称，门 blocking + 生成期有界修复兜住；人工批准路径只豁免 v8 及更早的存量稿——按落库 policyVersion 判定，v9 起照常复检，人工编辑路径不走生成期管线、批准门是唯一防线）；ranking 六家等长平行结构改为集合相等门（ADR-0009：逐家覆盖同一套 6 维、顺序不敏感、对照随文落库的注入清单，存量稿回退与第一家比对），第 1 家必须是目标品牌，第 2–6 家必须恰为五家已确认有效竞品，竞品内部顺序不限，workspace 自名、已确认别名和 relatedBrands 均不计入有效竞品；news 两类导语 5W1H、news 主体 3–4 递进小标题（≤8 字）；五类统一以最后一个独立小节收束、单独设「总结」小标题（2026-09-03 裁决，news 的总结小节不计入主体 3–4 个递进小标题；提示词层纪律，确定性门不校验）。
...
6. 正文恒注入品牌身份块（`renderBrandIdentityBlock`，实体层子集 + 简称白名单纪律；加粗纪律已移除，改管线自动加粗——ADR-0009；身份块同时承载指称序纪律——全称与已确认简称同存时首次全称、其后统一第一个简称，2026-09-03 裁决）；素材边界仍由 plannedFacts 圈定。`品牌：` 行与 direct 标题的品牌名取值与选题同口径（`resolveBrandName`：知识库身份事实优先，workspace 名仅兜底），不与已确认身份事实冲突。
...
8. 关键词融入（js_ai 已裁决语义回迁，v2）：全局基线约每 300 字 1 次；guide 每 500 字 1 次、news 每 300 字 1 次、news_light 每 200 字 1 次且密度 2%–5%；guide/ranking 首段嵌入 1–2 个关键词；地域/核心关键词首次出现及关键论据处加粗；列表项以「标签+内容」组织时标签加粗（优先冒号型 `- **标签**：内容`、冒号在加粗块外，漏写由管线 `autoBoldListLabels` 自动补全，2026-09-04 裁决 D22）；单一加粗实体（品牌名、ranking 维度名与列表项标签除外）≤3 次，H2 不加粗。
9. ranking 编排细则（js_ai 已裁决语义回迁，v2；维度来源 2026-09-01 改骨架注入；收束结构 2026-09-02 v8/D20、2026-09-03 精化——中间「段→部分」措辞微调曾于 main 线补登记为 v9/D21，已被同轮精化收纳取代）：维度由生成前维度选定小调用现选（lite 路由 `purpose: "dimension-planning"`，每篇现选、重试重发，解析校验 fail-loud）并字面注入正文 prompt，六家逐字共用；标题数字=正文陈列项数；六家陈列结束后写选型建议段（隐性条件式点首位，数字与陈列位 1 对账），随后以全文最后一个独立小节收束——单独设「总结」小标题、正文 80–150 字回扣主题并提示下一步判断，不与选型建议混写（2026-09-03 精化）；目标品牌最强维度置首、竞品两三条专精优势加一两条客观局限；总—分—总总领（引言只承担总览、不得使用 `##` 小节标题），全文 2500 字上限不变。
...
| D20 | ranking 收束结构 | 榜单文无总—分—总与独立总结段硬约束 | format 新增「总—分—总」总领与「引言禁 `##` 小节标题」两条（现有八条一字未动）；expression 选型建议由「全文倒数第三段」改为顺序语义（六家陈列结束后写选型建议段），并新增选型建议后独立 80–150 字总结段——2026-09-03 精化为全文最后一个独立小节、固定「总结」小标题、不与选型建议混写；同轮裁决泛化到五类：所有文章类型以最后一个独立小节收束、单独设「总结」小标题（news 的总结小节不计入主体 3–4 个递进小标题）；全文 2500 字上限不变；确定性审核门不新增校验。中间「段→部分」措辞微调曾于 main 线补登记为 v9/D21，被本行 2026-09-03 精化收纳取代。v8 覆盖 | 用户裁决 2026-09-02（2026-09-03 精化并泛化五类） |
| D21 | 品牌指称序 | 无对应机制（模型行文自由） | 正文品牌指称序裁决（首次全称、其后简称）：可检正文（标题行/围栏代码块/图片语法/链接 URL 盲区不计，与加粗门同源遍历）首个品牌指称必须逐字全称、其后统一钉 `shortNames[0]`、全文仅一次也用全称；门校验三个机械点（首个非全称、全称在首次后复现、首次全称后混用非第一个已确认简称——第三点为票 #44 评审修复补齐，原「白名单内其他简称不拦」使钉住只剩提示词层保证）blocking，生成期管线（含一次有界修复 pass）兜住；身份块 prompt 注入同句纪律；缺全称或缺已确认简称时规则不适用。人工批准路径（`approve`）对存量稿传 `brandNameOrderEnforced: false` 豁免，不追诉 v8 及更早稿件——追诉阈值保留 v9（票 #44 三评裁决：main 线 v9 是「段→部分」措辞微调、其 prompt 从未承载指称序，提到 10 会把本线 v9 稿静默豁免；两线 v9 磕撞中误拦可见可自纠、误豁静默失防，故从 9）。正文段 policyVersion v8→v9 | 用户裁决 2026-09-03 |
| D22 | 列表项标签加粗 | 无对应机制（ranking 维度名加粗为局部契约） | 「标签+内容」列表项的标签加粗推广为全类型管线保证：`autoBoldListLabels` 在 parse 后自动补粗，先于 `autoBoldBrandMentions` 执行（标签内含品牌名时整标签加粗、品牌加粗不嵌套）。两种形态：冒号型（优先，全/半角冒号都认，冒号留在加粗块外，与 ranking「- **维度名**：内容」同口径）与空格型（取首个空白前 token，空格保留原位不改原文结构）；标签约束 2–12 字、无句读标点、不含 `*[]()`` 或 http（已加粗/链接/代码不碰）、不纯数字/纯拉丁，空格型额外过停用词闸（我们/首先/注意等叙述句开头不是标签）且标签后必须有正文；行首允许 ✅ 等前置符号段，符号不进加粗块；盲区与品牌加粗同源（`brandMentionLines` 改名 `classifyBodyLines` 后四处消费方共用：标题行/围栏代码块整行跳过）。prompt 全局加粗规则同步声明该形态并把「单一加粗实体 ≤3 次」的豁免扩为品牌名、对比清单维度名与列表项标签；不新增 blocking 门，人工编辑路径不自动改写（与品牌加粗立场一致）。正文段 policyVersion v9→v10 | 用户裁决 2026-09-04 |
