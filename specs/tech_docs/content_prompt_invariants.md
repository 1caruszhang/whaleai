...
9. ranking 编排细则（js_ai 已裁决语义回迁，v2；维度来源 2026-09-01 改骨架注入；收束结构 2026-09-02 v8/D20；总结表述 2026-09-03 v9/D21）：维度由生成前维度选定小调用现选（lite 路由 `purpose: "dimension-planning"`，每篇现选、重试重发，解析校验 fail-loud）并字面注入正文 prompt，六家逐字共用；标题数字=正文陈列项数；六家陈列结束后写选型建议段（隐性条件式点首位，数字与陈列位 1 对账），随后独立 80–150 字总结部分收束（回扣主题、不与选型建议混段）；目标品牌最强维度置首、竞品两三条专精优势加一两条客观局限；总—分—总总领（引言只承担总览、不得使用 `##` 小节标题），全文 2500 字上限不变。
...
| D20 | ranking 收束结构 | 榜单文无总—分—总与独立总结段硬约束 | format 新增「总—分—总」总领与「引言禁 `##` 小节标题」两条（现有八条一字未动）；expression 选型建议由「全文倒数第三段」改为顺序语义（六家陈列结束后写选型建议段），并新增选型建议后独立 80–150 字总结段（不与选型建议混段）；全文 2500 字上限不变；确定性审核门不新增校验。v8 覆盖 | 用户裁决 2026-09-02 |
| D21 | ranking 结尾总结表述 | 「单独写一段 80–150 字的总结」 | 「结尾在选型建议之后再单独写一个 80–150 字的总结部分」（段→部分：允许节级收束，不强制单段；「不与选型建议混为一段」语义不变；确定性审核门不新增校验）。正文段 policyVersion v8→v9 | c6e4251 契约微调；2026-09-03 code-review 补登记 |
