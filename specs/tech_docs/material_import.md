...
- **联网形态（ADR-0007 源锚定抽取，两层名单）**：主路径 `searchSources`（豆包搜索 HTTP API，纯结构化召回 Title/Summary/Url，不经 LLM 改写）×2 次并行（「{主锚+产品赛道} 排行榜/哪家好」两形态互补，各 20 条；查询主语取具体产品（同赛道纪律），无产品才退行业伞词）→ 一次普通抽取调用（extraction 槽、不联网）从快照认名字，输出两层 `{direct, potential}`——direct=三同全中（同品类/同模式/同区域），potential=抢同一批客户但三同缺一角（同品类不同区域、同区域不同品类、品类标杆单店），判不准归潜在宁低勿高（ADR-0007 Decision 6，用户裁决 2026-08-30）；继承同层级/同赛道/前东家排除/自名形近排除/已知竞品去重与宁缺毋滥纪律。`enable_search` 合并式调用降级为 `searchSources` 不可用时的兜底（无快照可比，存在闸明确降级，仅直接层，inferred 必审不变）。
