...
- **候选形态（ADR-0007 元数据退役 + 两层名单）**：两层各自纯名称数组、各自 inferred 低置信必审——直接层落 `competitors`（与本次已抽出的品牌 scope 竞品合并为同一条候选，避免同一 fact key 多条候选顺序采纳互相覆盖），潜在层落 `potentialCompetitors`（第 16 字段，同样与材料抽出值合并）；确认卡两行分开陈列，潜在行带补位小注，均逐项 ✕。`[[xiaojing-competitor-details:v1]]` 审计头停止新增编码（读侧保留解码兼容存量）；证据文本行「名（地域）：快照」随摘录留痕供展开复核。提议 value 只含本次新增名称：既有权威值由 KnowledgeAuthority propose 的数组增量合并契约保住。
