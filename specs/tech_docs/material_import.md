...
- **联网形态**：竞品补全只调用 `keyword-search.search()`，由 ARK `enable_search` 联网模型生成可核验语料；不调用 `searchSources`/web_leg，也没有两条路径之间的回退分支。每条联网 prompt 都携带目标品牌、地域、具体业务、四条竞品标准和「最多 10 家」结构要求，再分别以本地排行榜、口碑同行、品牌点名对手三种检索重点互补；逐 query 容错、结果拼接。三条全失败时保留空竞品必审行，不启用 web_leg 硬回退。
- **联网证据门**：候选名和 `sourceExcerpt` 必须逐字存在于联网模型返回的语料，同一短句附近必须有竞品/竞争/同行/对比/推荐/口碑或排行榜/排名/榜单/十佳/Top 等同类选择信号。榜单信号只是关系门的一部分；候选仍必须同时通过地域、赛道、体量、自名/关联品牌排除及用户确认，不得把榜单里的设备商或跨层级品牌直接当竞品。
...
- **反虚构与自名过滤**：只允许输出在联网语料中**字面出现**的公司/品牌名，且 `sourceExcerpt`、地域、具体同类业务都必须逐字存在于语料；地域和业务还必须与该竞品名在同一句的 120 字上下文内共现，不能借用另一家的地域/业务，也不能由模型改写出虚假竞争关系。排除名单（品牌自身/别名/关联主体）按双向子串匹配。数量补足缺口即止。
...
- **失败语义与必审行**：检索或富化解析失败不拖垮其他字段，也不得让竞品类从第一张事实确认卡消失。即使模型返回零条其他事实，仍继续联网腿；材料腿与联网腿均无合格名称时提交 `competitors=[]` 的 inferred 候选，excerpt 明示「请用户确认并补充」；已有权威竞品时投影现值。空数组只表达候选不足，绝不以错误主体凑数，后续 ranking 数量门仍 fail-closed。
