...
- **联网形态（ADR-0007 源锚定抽取）**：主路径 `searchSources`（豆包搜索 HTTP API，纯结构化召回 Title/Summary/Url，不经 LLM 改写）×2 次并行（「{主锚+行业} 排行榜/哪家好」两形态互补，各 20 条）→ 一次普通抽取调用（extraction 槽、不联网）从快照认名字，输出 `{name, region}`；继承同层级/同赛道/前东家排除/自名形近排除/已知竞品去重与宁缺毋滥纪律。`enable_search` 合并式调用降级为 `searchSources` 不可用时的兜底（无快照可比，存在闸明确降级，inferred 必审不变）。
