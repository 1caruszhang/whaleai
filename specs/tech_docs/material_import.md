...
- **联网形态（ADR-0007 源锚定抽取）**：主路径 `searchSources`（豆包搜索 HTTP API，纯结构化召回 Title/Summary/Url，不经 LLM 改写）×2 次并行（「{主锚+产品赛道} 排行榜/哪家好」两形态互补，各 20 条；查询主语取具体产品（同赛道纪律），无产品才退行业伞词）→ 一次普通抽取调用（extraction 槽、不联网）从快照认名字，输出 `{name, region}`；继承同层级/同赛道/前东家排除/自名形近排除/已知竞品去重与宁缺毋滥纪律。`enable_search` 合并式调用降级为 `searchSources` 不可用时的兜底（无快照可比，存在闸明确降级，inferred 必审不变）。
- **本地三闸（零 API）**：存在闸——name 归一化后逐字见于快照语料（恒开，兜住抽取漏网幻觉）；地域闸——仅城市/区县锚执行（region 与白名单双向去后缀包含比对，拦跨城错配），省级锚不拦；关系闸——所引快照里名字附近出现供应/合作/前东家等非竞争关系词的剔除（NON_COMPETITOR_RELATION 词表）。无锚（serviceArea 缺失或「全国/线上」类）整轮跳过，不联网。
- **候选形态（ADR-0007 元数据退役）**：富化名一律 `inferred` 低置信必审，与本次已抽出的品牌 scope 竞品**合并为同一条候选**（避免同一 fact key 多条候选顺序采纳互相覆盖）；事实值与确认卡只承载**纯名称**，`[[xiaojing-competitor-details:v1]]` 审计头停止新增编码（读侧保留解码兼容存量）；证据文本行「名（地域）：快照」随摘录留痕供展开复核。提议 value 只含本次新增名称：既有权威值由 KnowledgeAuthority propose 的数组增量合并契约保住。
