...
- **画像注入**：联网模型判别四条件需要目标品牌画像——products/serviceArea 取本次材料值，缺失时用 KnowledgeAuthority 已确认权威值补齐（predicate 按小写化契约 `enterprise-profile.servicearea`）；都没有时提示词声明未知并收紧判别（名字必须与本地门店/服务商语境共现）。
- **联网形态（合并式）**：竞品补全只调用 `keyword-search.search()`，由 ARK `enable_search` 联网模型在**同一次调用内完成检索与结构化抽取**（直接返回候选 JSON），不再有独立的二次抽取级——旧两级链路对 LLM 改写后的语料做逐字证据校验近乎不可通过，冷门品牌必然 0 候选。不调用 `searchSources`/web_leg，也没有两条路径之间的回退分支。每条联网 prompt 携带目标品牌画像、四条竞品标准与 JSON 结构要求，分别以本地排行榜、口碑同行、品牌点名对手三种检索重点互补；逐 query 容错（检索异常或非 JSON 响应单独跳过），跨 query 聚合去重、数量补足缺口即止。三条全失败时保留空竞品必审行，不启用 web_leg 硬回退。
- **轻过滤（替代旧逐字证据门）**：合并式下检索语料与候选同源，逐字校验不再适用；`parseCompetitorSuggestions` 收窄为——结构校验（name/region/similarBusiness/sourceExcerpt 四项齐备）+ 摘录内非竞争关系排除（前东家/合作/供应/经销/客户/设备品牌等关系词与名字近邻即剔除，拦下把上下游改写成竞品的常见错误）+ 自名/关联品牌排除（双向子串 + `isSimilarSelfName` 形近变体）+ 已知竞品/跨 query 去重。质量纪律（层级/赛道/地域/榜单语料警示）由提示词承担，最终裁决在确认卡——富化名一律 inferred 低置信必审，不得自动确认。
- **富化合并提示词**：目标品牌画像块（品牌/行业/核心产品/服务区域/体量层级）+ 四条件判别标准（同赛道明确「看具体产品/服务，不看行业大类」）+ 榜单语料警示（「国家/地区 + 品牌 + 英文名」的榜单行文是国际品牌条目，一律不取；散文/品类/评价语不是企业专名）+ 已知竞品与排除名单 + 检索重点 + 反虚构纪律（只允许输出经检索确认真实存在的名字，不得凭记忆输出检索未提及的）+ 宁缺毋滥（没有同层级本地同行返回空数组）。
- **反虚构落点**：从「逐字存在于语料」改为「检索确认真实存在 + 确认卡裁决」。幻觉名的风险由卡片上逐条「采用/保留/拒绝」与三元组展示兜住（用户可按 名称｜地域｜业务 判断），不再让 0 候选把补名成本全部转嫁给聊天手打。
- **候选形态**：富化名一律 `inferred`（卡片「待确认」行；整卡确认时随全量采纳进入权威，见 ADR 0003）；每家必须同时有「竞品名称、所在地域、具体同类业务」，确认卡按 `名称｜地域｜业务` 组合展示；与本次已抽出的品牌 scope 竞品**合并为同一条候选**（名称按小写归一去重），避免同一 fact key 多条候选顺序采纳互相覆盖。提议 value 只含本次新增名称：既有权威值由 KnowledgeAuthority propose 的数组增量合并契约保住（current 在前、新增去重追加），已确认名称不以「待确认」形态重复呈现。权威值仍只保存竞品名数组；展示元数据以版本化审计头附在候选 excerpt 中（审计头与证据文本总长按 KnowledgeAuthority 的 excerpt 长度闸门预算内自截），卡片投影时解码且不把元数据标记暴露给用户——头部畸形/截断时丢弃元数据、只回退纯证据文本，宁可少展示也不泄漏标记。
- **失败语义与必审行**：检索或富化解析失败不拖垮其他字段，也不得让竞品类从第一张事实确认卡消失。固定码语义：全部联网调用异常/空响应 → `search_corpus_empty`；有响应但全部不是合法 JSON → `model_response_invalid`；合法响应但无候选过轻过滤 → `no_qualified_suggestions`。即使模型返回零条其他事实，仍继续联网腿；材料腿与联网腿均无合格名称时提交 `competitors=[]` 的 inferred 候选，excerpt 明示「请用户确认并补充」；已有权威竞品时投影现值。空数组只表达候选不足，绝不以错误主体凑数，后续 ranking 数量门仍 fail-closed。
