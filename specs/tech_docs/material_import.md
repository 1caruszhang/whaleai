...
- **失败语义与必审行**：检索或富化解析失败不拖垮其他字段，也不得让竞品类从第一张事实确认卡消失。固定码语义：无搜索能力 → `keyword_search_unavailable`；无地域锚 → `service_scope_missing`；已知竞品已满 → `deficit_zero`；searchSources 两条全失败/空 → `search_corpus_empty`；抽取响应坏 JSON（同信号内重抽一次）→ `model_response_invalid`；合法响应但无候选过闸 → `no_qualified_suggestions`。材料腿与联网腿均无合格名称且无既有权威竞品时提交 `competitors=[]` 的 inferred 候选，excerpt 以被动说明（如「材料未提供可定位的服务区域，竞品联网补全已跳过」）提示原因，不追问。空数组只表达候选不足，绝不以错误主体凑数，后续 ranking 数量门仍 fail-closed。
