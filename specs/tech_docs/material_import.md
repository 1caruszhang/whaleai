...
- **联网形态（ADR-0007 源锚定抽取，两层名单）**：主路径 `searchSources`（豆包搜索 HTTP API，纯结构化召回 Title/Summary/Url，不经 LLM 改写）×2 次并行（各 20 条；查询词优先用材料抽取同响应顺手产出的 `competitorSearchQueries`——模型以目标客户口吻写（经营者→项目/加盟池，终端消费者→榜单/口碑池），管线瞬时值不落库，缺失回落「{主锚+产品赛道} 排行榜/哪家好」默认形态；两条必须分池互补——第 1 条客户口吻需求问句型（…哪家好/怎么选），第 2 条中立观察者品类盘点型（…品牌有哪些/盘点）且禁带加盟/招商/合作词（2026-08-31 双池裁决），合并语料先按 URL 去重、再按可注册域每域封顶 3 条保检索序——防单一品牌 GEO 投放霸屏软文站把多品牌列表页挤出语料（用户裁决 2026-08-31「1+2」，张仔纪霸屏回归；域名键剥 www./m. 前缀与子域，com.cn 等两段公共后缀取倒数三段））→ 一次普通抽取调用（extraction 槽、不联网）从快照认名字，输出两层 `{direct, potential}`——direct=三同全中（同品类/同模式/同区域），potential=抢同一批客户但三同缺一角（同品类不同区域、同区域不同品类、品类标杆单店），判不准归潜在宁低勿高（ADR-0007 Decision 6，用户裁决 2026-08-30）；判别第 0 步为客户口径——画像注入目标客户/经营场景/核心优势，竞品=争夺同一批客户预算的对手，客户群不同的候选（如目标客户是创业者、候选服务终端消费者）两层都不进；继承同层级/同赛道/前东家排除/自名形近排除/已知竞品去重纪律；数量端为认全展示（用户裁决 2026-08-31：快照里所有能识别的真实品牌全部上报——分层只是标签，宁可多报待用户逐项删除，不自审漏报；仍禁幻觉名/排除名单/非竞争关系/平台渠道/国际榜单行文）；名字净化（繁归简、描述短语剔除）与层内/跨层嵌套互斥在解析层执行。`enable_search` 合并式调用降级为 `searchSources` 不可用时的兜底（无快照可比，存在闸明确降级，仅直接层，inferred 必审不变）。
