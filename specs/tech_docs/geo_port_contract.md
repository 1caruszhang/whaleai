...
| 3 fallback   | 结构化类目/人群/官方 GEO 标记到已批准资源池的保底召回 | 0.3 |
| 4 preference | 人工维护偏好清单到已批准资源池     | 0.1  |

四路按 resource id 求并集并只累加不同路径权重，再与真实超级媒介资源池相交。匹配先按注册域 eTLD+1，再按中文名称兜底；名称阈值 0.55，passive 域命中最多 3 个、active 最多 2 个、纯名称兜底最多 1 个。passive 每问题最多 15 条、同注册域最多 3 条；fallback 最多 50 条。fallback 触发=行业类目码 ∨ 人群词 ∨ 官方 GEO 标记（`geo_platforms` 非空；备注关键词只作信号不触发，2026-08-27），reference 形态 `industry:`/`audience:`/`geo:平台列表`（geo 载荷与快照 geoPlatforms 一致，Rust 确认门交叉校验）。

质量过滤在对齐和比例分配之前执行：发布率不参与决策；价格按统一公式换算为点数，并不得超过计划创建时冻结的用户“每篇文章最高点数”（默认 `3000` 点）。自动分配还受冻结的“每次分发最高点数”（默认 `20000` 点）约束。最终最多 30 个，媒体 / 自媒体目标配额 20 / 10；一侧不足时剩余名额流向另一侧。排序依次为路径加权分、命中路数、保底分档 tie-break（垂类∩官方GEO > 纯垂类 > 纯官方GEO > 仅人群词，同权重同命中数时生效，2026-08-27）、名称。
