...
五类内容的唯一集合是 `guide / showcase / ranking / news / news_light`。旧注释中的“四类”“六类”以及旧调研里的“每主题最多三类”均不是当前代码事实；当前实现允许每主题 1–5 类，并对整批内容类型覆盖做下限补齐（guide/ranking 各 ≥3、其余各 ≥2，合计 ≥12 篇；用户裁决 2026-08-26）。
...
| 3 fallback   | 结构化类目/官方 GEO 标记到已批准资源池的保底召回（垂类∩GEO 全量 + 纯垂类/单GEO 随机采样） | 0.3 |
| 4 preference | 人工维护偏好清单到已批准资源池     | 0.1  |

四路按 resource id 求并集并只累加不同路径权重，再与真实超级媒介资源池相交。匹配先按注册域 eTLD+1，再按中文名称兜底；名称阈值 0.55，passive 域命中最多 3 个、active 最多 2 个、纯名称兜底最多 1 个。passive 每问题最多 15 条、同注册域最多 3 条。fallback 召回（2026-08-28 三轮用户裁决：随机采样下沉召回层，合并层纯加权排序、无保底占位）= 垂类∩官方 GEO（t0）确定性全量 + 纯垂类（t1）每次随机补足 26 席（数据飞轮探索臂，未抽中即本次未被召回、不带 0.3 权重）+ 单 GEO（t2）随机补足至 30 封顶；人群词不再触发保底证据。fallback 触发=行业类目码 ∨ 官方 GEO 标记（`geo_platforms` 非空；备注关键词只作信号不触发，2026-08-27），reference 形态 `industry:`/`geo:平台列表`（三轮裁决起不再产生 `audience:` 引用，Rust 确认门仍兼容旧值；geo 载荷与快照 geoPlatforms 一致，交叉校验）。

质量过滤在对齐和比例分配之前执行：发布率不参与决策；价格按统一公式换算为点数，并不得超过计划创建时冻结的用户“每篇文章最高点数”（默认 `3000` 点）。自动分配还受冻结的“每次分发最高点数”（默认 `20000` 点）约束。最终最多 30 个，媒体 / 自媒体目标配额 20 / 10；一侧不足时剩余名额流向另一侧。排序依次为路径加权分、命中路数、保底分档 tie-break（垂类∩官方GEO > 纯垂类 > 纯官方GEO > 仅人群词 > 无规则命中，同权重同命中数时生效，2026-08-27；三轮裁决起人群词不再触发保底证据，后两档仅对经其它路进入的候选生效）、名称。
