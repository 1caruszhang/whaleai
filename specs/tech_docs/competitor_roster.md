...
① searchSources × 2 次，并行（豆包搜索 HTTP API，纯结构化召回 Title/Summary/Url，
   不经 LLM 改写，免费额度）。头部源随后抓网页正文并进语料：列表文常被
   引擎摘要截断在第 1 家（常是品牌自身投放），第 2~N 家在截断线下——前 8
   条各抓正文（截 6000 字、8s 超时、失败静默跳过，用户裁决 2026-08-31
   「正文全部读取」；初版 1500 字在第六写实跑复切一刀——6 家盘点文的第
   5/6 家落在第 1658/1990 字，卡上只剩 3 家，按实测列表文剥标签后
   2000～5400 字提到 6000 字覆盖全文）；关系闸仍用摘要文本（列表文正文满是招商措辞，按
   正文判关系会整页误杀），只在正文命中的名字证据从命中处就近截取；查询词优先用材料抽取时模型以【目标客户口吻】
   ...
   公共后缀取倒数三段）
② 一次普通 LLM 抽取调用（extraction 槽，如 DeepSeek，不联网；输出前同名自检——
   同品牌不同写法只报一次，模型先判、③的身份判定兜底；数量端认全展示
   （用户裁决 2026-08-31）：快照里所有能识别的真实品牌全部上报，分层只是
   最佳判断标签，宁可多报待用户逐项删除，不自审漏报）：
...
③ 本地字符串校验（零 API 调用），两层同闸：
   ...
   - 名字净化：繁体源页名归简存储（榕邊→榕边，存在闸比对两侧同映射）；
     同品牌身份判定 sameBrandIdentity：归一键嵌套（括号中缀剥离）+「·」
     分段交叉（地域段经 regionHints 剔除）；存储名忠实原报不截断
     （张仔纪（广州）…/张仔纪…、张仔纪·老顺德干蒸菜/张仔纪干蒸菜、
     顺德·渔文乐/渔文乐 各自同名只留一份，顺德·渔文乐≠顺德杨廷记）；引号包裹/「相关」句式的描述短语
     不是品牌名，剔除
④ 幸存者作为 inferred/0.5 候选走既有 propose → 确认卡（数据两个 fact：
   ...
   URL：「名（地域）：60 字上下文（来源：<url>）」（证据瘦身 2026-08-31：来源链接可点后 200 字内联证据冗余，每段压到约 120 字）；卡面上每个品牌胶囊旁直接渲染「来源」小链接（用户裁决 2026-08-31：链接不埋进展开长文），展开后的依据段同样链接化；卡片投影对竞品类摘录放宽截断上限至 2000 字（每段证据+URL 约 300 字，通用 300 字上限曾把第 2 家起的 URL 全部截掉——实跑 6 家只剩第 1 家有链接）
```
...
- **两段式自适应补枪（第五写实跑裁决）**：两层幸存合计 <2 视为池太薄——重写一条去场景/去招商的纯品类盘点词（模型看召回标题事实反馈式重写，`parseRetryQuery` 拒空/重复/招商词），换池补一枪后合并语料重新抽取重新过闸，两轮幸存按 sameBrandIdentity 并集；任何一步失败保第一轮结果。出口 `path:main|retry` 分轮留痕。
