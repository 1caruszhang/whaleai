...
`src/shared/systemReminder.ts` 是唯一 envelope builder。所有动态字段先做 XML escaping，数字归一为非负整数；payload 只携带 artifact/operation 的结构化标识、revision、数量与状态（如 planId、articleId、approvedRevision、assignmentCount），不复制正文或 secret。

决策回执覆盖五个确认门，指令语义一致：「从权威结果继续，不重复询问已裁决内容」：
