...
> 状态：立项票已落地（4e45b39，分支 geo/artifact-lineage-owner——owner＋守卫棘轮 27 项豁免＋删除原因词表双侧 pin，纯增量全绿）；清零样板票 02 baseline 已落地（60d2aaf——2 写点经 owner、from 规则按真实代码钉进 `allowed_from`、豁免表消 2 项余 25，等价全绿 cargo 472＋npm 156）；清零票 03 question-pool（65e7dfe）与 05 distribution（08f1348——3 写点经 owner、镜像链 discovering→draft|unavailable→confirmed from 规则按真实代码钉、豁免表消 3 项，等价全绿）已落地；清零票 06 articles 已落地（bb6e400——2 写点经 owner、聚合驱动 from 规则按真实代码钉（claim 不写血缘行，completed→completed-with-failures 等多对多如实登记、completed 只从 running）＋聚合源状态→血缘态端到端钉、豁免表消 2 项余 17，等价全绿 cargo 477＋npm 156）；清零票 04 topic-plan 已落地（345c883——2 写点经 owner（create 开行＋confirm 迁移）、awaiting-confirmation 空 from 集（open-only，沿票 03 惯例）＋confirmed 自 {awaiting-confirmation, confirmed}（再确认重写终态，复用停卡重选真实可达）、豁免表消 2 项余 15，等价全绿（隔离工作树按本票树形 cargo --lib 477＋守卫 9 绿））；清零票 07 monitor 已落地（52386d2——6 写点经 owner（prepare 开行＋activate/two 终局/pause/resume 五处 set 迁移，0 行 no-op 语义经 optional 读取逐位保持）、from 规则按真实代码钉（draft 空 from 集、paused 只自 active、active 自 {draft,paused,completed}、completed 自 {active,paused}——各含一条镜像破裂边：末单元余额不足时尾随 refresh 的终局写不看计划门把血缘推到 completed（计划留 paused）、余额恢复后 resume 只门卫计划 paused 即 completed→active；票面猜想的 completed←draft 经真实代码证伪——两处终局写均只在计划 active 后可达）、端到端钉先在旧直写代码上验证钉住＋豁免表消 6 项余 9，等价全绿（隔离工作树按本票树形 cargo --lib 479＋守卫 9 绿））；收官票 08 publish 已落地（afab958——9 写点经 owner（prepare 两处废弃＋开行走 open_lineage，confirm/start/复活/取消/恢复/聚合刷新走 mirror_publish_lineage→set_lineage_state，执行行缺失保持旧子查询 0 行被忽略 no-op）、from 规则按真实代码钉 11 态（含两处真实镜像破口：prepare 幂等键冲突分支翻转执行 status 而不写血缘——resume 对翻转执行照常放行故 scheduled 的 from 集囊括八态；聚合刷新无条件重写血缘——changed==0 与 refresh_all 周期重扫也照写，五个聚合态含自环；取消后在途单 settle 的 cancelled 漂移边如实登记；running 的 from 集＝聚合可见全集——terminal_failed 不单调（retry 重置收回条目），partial→running 真实可达，该回归由 code-review 轴发现并于同票修复＋端到端钉 republish_resets_partially_succeeded_execution_back_to_running 钉住）、豁免表清空（9→0）——棘轮终态达成：七域 27 处直写全部经 owner，守卫零豁免；原「未钉族无校验」测试职责消灭，替换为棘轮终态钉（31 态全部有 from 规则，漏登记者必红）；等价全绿 cargo --lib 481＋npm test 156（守卫 9 绿零豁免））——全部八张票结案；2026-09-04 独立验收绿（架构会话复跑：npm test 156 全过＋cargo test 481＋1＋1 全绿；红线复核——geo_operations.rs 在 f891068..HEAD 零改动、守卫豁免表空集严格相等、契约两键 10+4 双侧 pin 在位、31 态 from 规则终态钉在位）
