...
> = {
  ...
  ranking: {
    format: [
      "采用「总—分—总」结构：开篇引言总述行业格局 → 中间六家并列清单分述 → 结尾选型建议与总结收束。",
      "引言只承担总览功能，不得使用 `##` 小节标题。",
    ...
    expression: [
      ...
      "结尾在选型建议之后单独写一段 80–150 字的总结：回扣主题、提示读者如何根据本文信息做下一步判断，不与选型建议混为一段。",
...
export function buildArticleGenerationMessages(input: {
  ...
  const system = [
    ...
    // 内容可信度与结构化表达纪律（D19，v8）：EEAT 与知识图谱原则的写作
    // 纪律转写，不出现两组原字样；纯提示词层尽力约束，不新增审核项；权
    // 威信号的竞品表述限定对比清单类型，避免与非 ranking 类型的竞品禁令打架。
    "【内容可信度与结构化表达纪律】生成正文时，在不编造、不泄露内部实现名称、不贴标签的前提下，自然满足以下四重信号，并让核心概念可被清晰抽取：",
    "- 经验信号：用具体场景、真实服务流程、可验证细节或典型做法支撑叙述，避免纯概念堆砌；能用「谁在什么情况下怎么做」说明的，不写抽象自评。",
    "- 专业信号：以行业知识、判断标准或方法论组织内容（如选型维度、服务流程、评估框架），把品牌呈现为「懂行的人」而非口号式宣传。",
    "- 权威信号：关键主张紧靠已批准事实，数字、资质、案例、服务范围只来自输入的事实清单；若本篇为对比清单且涉及竞品对比，竞品只用公开可核验的经营事实，不虚构来源或具体数据。",
    "- 可信信号：不夸大、不绝对化；涉及效果、价格、服务边界等敏感点时，用条件句与限定词（如「若…则…」「适用于…」）降低误导风险；事实不足时选择省略而非填充。",
